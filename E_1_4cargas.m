...
% Campo Q3 (Placa negativa)
rx = xGrid -(movX);
ry = yGrid - 0;
rz = zGrid -lz;
r = sqrt(rx.^2 + ry.^2 + rz.^2); % distancia de la particula (magnitud)
Ex3 = ((-q / 4*pi*eps0) * rx) ./ r.^3; % E = [(k*q) / sqrt ( (x-qx)^2 + (y-qy)^2 + (z-qz)^2) )^2 ] * r unitario
Ey3 = ((-q / 4*pi*eps0) * ry) ./ r.^3;
Ez3 = ((-q / 4*pi*eps0) * rz) ./ r.^3;

% Campo Q4 (Placa negativa)
rx = xGrid -(movX);
ry = yGrid - 0;
rz = zGrid - (-lz);
r = sqrt(rx.^2 + ry.^2 + rz.^2); % distancia de la particula (magnitud)
Ex4 = ((-q / 4*pi*eps0) * rx) ./ r.^3; % E = [(k*q) / sqrt ( (x-qx)^2 + (y-qy)^2 + (z-qz)^2) )^2 ] * r unitario
Ey4 = ((-q / 4*pi*eps0) * ry) ./ r.^3;
Ez4 = ((-q / 4*pi*eps0) * rz) ./ r.^3;
...
scatter3(-movX,0,lz,'g','linewidth',5)
scatter3(-movX,0,-lz,'g','linewidth',5);
scatter3(movX,0,lz,'g','linewidth',5)
scatter3(movX,0,-lz,'g','linewidth',5);
